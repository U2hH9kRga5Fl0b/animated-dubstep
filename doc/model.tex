...
First, we wanted to test that each operation followed every previous operation, and that every in-size matched every out-size. 
We conceived this as calculating the the total time and distance for each driver by using a different matrix. 
After discussions with the group, however, we figured out another way to represent all of the solution information in one single matrix. 

This amounts to representing a route not as indicies into the city, but as a table of all the components of an action put into a table for a given driver.
This is convenient to check the constraints because it is more simple to know the next operation from the current stop.
However, this means that solutions duplicate data from the city, and it is possible to give a solution matrix that is inconsistent with the city.

\subsection(City Streets)

Originally, there was also an idea of coding every city street into the city objects, so that there were multiple ways to reach every destination.
This would simulate a real city's data. 
After analyzing this method we determined that each driver would simply choose the shortest path if given the option, so we would just have the shortest path connecting every location.
This ended up simplifying the problem, and allowed the simulator better performance.


\subsubsection{Modeling Language}
\subsubsection{Simulator output}

\subsubsection{Other decisions we spent time on}
MATLAB supports objects?
include number of requests in city object?
how many things should go into one big matrix vs several small ones?
directed graph.
not using direct paths, but just giving streets and including which streets as part of solution.
